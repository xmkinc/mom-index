...
`sample_quality` 还携带可选的机器可读门槛证据 `gates`（固定顺序，最多 12 条）。每条门槛只包含六个字段：规范原因代码 `code`、`level`（`low` 表示未通过即触发低置信度，`high` 表示未通过即无法达到高置信度）、`passed`、实际聚合值 `actual`、门槛值 `threshold` 和 `comparator`（`gte`/`lte`，按 `actual >= threshold` 或 `actual <= threshold` 判定）。样本量为整数，比例四舍五入到四位小数；`empty_sample` 是规范原因代码，但不生成数值门槛。`gates` 是纯加法字段：置信度判定、原因代码和其余样本质量字段与之前完全一致；旧 payload 没有 `gates` 时页面回退到原因代码的中文标签；门槛列表格式异常时导出只省略该列表，不丢弃其余样本质量字段。页面在门槛证据可用时用中文展示每个未通过门槛的实际值与阈值，并附上一段只来自公开汇总字段的证据说明；未知原因代码会原样显示，不会被隐藏。前端标签与后端规范代码的一致性由测试强制：任何规范代码缺少前端标签都会使测试失败。
